'use strict';
const http = require('https');
const fs = require('fs');
const path = require('path');
const zlib = require('zlib');
// TODO: get rid of dependency
const tar = require('tar');


class Addon {
  constructor() {

  }

  _setupBuildDir(cb) {
    let count = 1
    const buildDirs = [
      `${process.cwd()}/build`,
      `${process.cwd()}/build/deps`,
      `${process.cwd()}/build/deps/node`,
      `${process.cwd()}/build/deps/node/${process.versions.node}`,
      `${process.cwd()}/build/Release`,
      `${process.cwd()}/build/Debug`
    ]
    buildDirs.forEach((dir) => {
      fs.mkdir(dir, (err) => {
        // ignore error cases: existing folders; TODO: handle other cases
        if (count >= buildDirs.length)
          return process.nextTick(() => { cb(null) })
        else
          count++
      })
    })
  }

  _download(url, output, cb) {
    const options = {
      'method': 'GET',
      'hostname': 'nodejs.org',
      'path': url,
      'headers': {}
    };
    const req = http.request(options, function (res) {
      if (path.parse(url).ext === '.gz') {
        res.pipe(zlib.createUnzip())
           .pipe(tar.Extract({path: `${process.cwd()}/build/deps/node/${process.versions.node}`, strip: 1}))
      } else {
        res.pipe(fs.createWriteStream(output))
      }
      res.on('end', function () { return process.nextTick(() => { cb() }) });
      res.on('error', function (err) { return process.nextTick(() => { cb(err) }) });
    })
    req.end()
  }

  _copyRCExe() {
    fs.stat(`${process.cwd()}/build/rc.exe`, function(err, stat) {
      if (err == null) {
        // just ignore the "file exists case"
      } else {
        let arch = ''
        switch (process.arch) {
          case 'x64': arch = 'x64'; break;
          case 'ia32': arch = 'x84'; break;
          case 'arm': arch = 'arm'; break;
        }
        const rc = fs.readFileSync(`%programfiles(x86)%\\Windows Kits\\8.1\\bin\\${arch}\\rc.exe`);
        const rcdll = fs.readFileSync(`%programfiles(x86)%\\Windows Kits\\8.1\\bin\\${arch}\\rcdll.dll`);
        // we don't handle the callback except throwing an error
        fs.writeFile(`${process.cwd()}/build/rc.exe`, rc, (err) => { if (err) throw err; });
        fs.writeFile(`${process.cwd()}/build/rcdll.dll`, rcdll, (err) => { if (err) throw err; });
      }
    });
  }

  getDeps(cb) {
    let count = 1
    const r = process.release
    const urls = [ {name: 'headers', url: r.headersUrl} ]
    // only present on win32
    if (process.platform === 'win32') urls.push( {name: 'node.lib', url: r.libUrl} )
    // first check whether download is necessary
    this._setupBuildDir((err) => {
      if (err) return cb(err)
      // fire and forget racy rc.exe copy
      if (process.platform === 'win32') _copyRCExe()

      fs.readdir(`${process.cwd()}/build/deps/node/${process.versions.node}/include/node`, (err, files) => {
        // Ignore the error case here, since we'll then override.
        // If the header dir has at least one file we assume it has them all
        if (files && files.length > 1)
          return process.nextTick(() => {cb(null)})

        urls.forEach((el) => {
          this._download(el.url, `${process.cwd()}/build/deps/node/${process.versions.node}/${el.name}`, (err) => {
            if (err) return cb(err)
            // invoke callback only when all deps have been downloaded or err'd
            if (count >= urls.length)
              return process.nextTick(() => { cb(null) })
            else
              return process.nextTick(() => { count++ })
          })
        })
      })
    })
  }
}

module.exports = new Addon()
