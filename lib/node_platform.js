  ...
  _copyRCExe() {
    fs.stat(`${process.cwd()}/build/rc.exe`, function(err, stat) {
      if (err == null) {
        ...
        fs.readFile(`${process.env['ProgramFiles(x86)']}\\Windows Kits\\10\\bin\\${arch}\\rc.exe`, (err, file) => {
          if (err) {
            fs.readFile(`${process.env['ProgramFiles']}\\Windows Kits\\10\\bin\\${arch}\\rc.exe`, (err, file) => {
              if (err) throw err
