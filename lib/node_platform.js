  ...
  _copyRCExe() {
    fs.stat(`${process.cwd()}/build/rc.exe`, function(err, stat) {
      if (err == null) {
        ...
        const rc = fs.readFileSync(`${process.env['ProgramFiles(x86)']}\\Windows Kits\\10\\bin\\${arch}\\rc.exe`);
        const rcdll = fs.readFileSync(`${process.env['ProgramFiles(x86)']}\\Windows Kits\\10\\bin\\${arch}\\rcdll.dll`);
