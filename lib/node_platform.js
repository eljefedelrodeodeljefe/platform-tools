  ...
  _copyRCExe() {
    fs.stat(`${process.cwd()}/build/rc.exe`, function(err, stat) {
      if (err == null) {
        ...
        fs.readFile(`${process.env['ProgramFiles(x86)']}\\Windows Kits\\10\\bin\\${arch}\\rc.exe`, (err_1, file) => {
          if (err_1) {
            fs.readFile(`${process.env['ProgramFiles']}\\Windows Kits\\10\\bin\\${arch}\\rc.exe`, (err_2, file) => {
