...
const vsStdIncludePaths = [
  '"%programfiles(x86)%\\Windows Kits\\10\\Include\\10.0.10240.0\\ucrt"', // REVIEW
  '"%programfiles(x86)%\\MSBuild\\Microsoft.Cpp\\v4.0V140"',
  ...
  '"%programfiles(x86)%\\Windows Kits\\8.1\\bin\\arm"',
  '"%programfiles(x86)%\\Windows Kits\\8.1\\lib\\arm"',
  '"%programfiles(x86)%\\Windows Kits\\8.1\\lib\\winv6.3\\um\\x86"',
  '"%programfiles(x86)%\\Windows Kits\\8.1\\lib\\winv6.3\\um\\x64"',
  '"%programfiles(x86)%\\Windows Kits\\8.1\\lib\\winv6.3\\um\\arm"',
  ...
  '"%programfiles(x86)%\\Microsoft Visual Studio 14.0\\VC\\atlmfc\\include"',
  '"%programfiles(x86)%\\Microsoft Visual Studio 14.0\\VC\\include"',
  '"%programfiles(x86)%\\Microsoft Visual Studio 14.0\\lib"',
  '"%programfiles(x86)%\\Microsoft Visual Studio 14.0\\lib\\amd64"',
  '"%programfiles(x86)%\\MSBuild\\Microsoft.Cpp\\v4.0\V140"',
  '"%programfiles(x86)%\\MSBuild\\Microsoft.Cpp\\v4.0"',
  '"%programfiles(x86)%\\Windows Kits\\8.1\\Include\\um"',
  '"%programfiles(x86)%\\Windows Kits\\8.1\\Include\\shared"',
  '"%programfiles(x86)%\\Windows Kits\\8.1\\Include\\winrt"'
...
class PlatformTools extends EventEmitter {
  ...
  compile(source, opts, cb) {
    ...
    if (process.platform === 'win32')
      vsStdIncludePaths.forEach((el) => { args.push(`/I${el}`) })
  ...
  link(objectFile, opts, cb) {
    ...
    if (process.platform === 'win32')
      vsStdIncludePaths.forEach((el) => { args.push(`/LIBPATH:${el}`) })
    ...
    if (options && options.output) {
      ...
      if (process.platform === 'win32') {
        args.push(`/OUT:${filename}${options.isWinAddon ? '' : '.exe'}`)
      } else {
        args.push('-o')
        args.push(`${filename}`)
      }
    ...
    addon.getDeps((err) => {
      ...
      if (process.platform === 'darwin') {
        ...
        opt.compiler_flags = [ '-fPIC', '-pthread', '-Wall', '-Wextra',
          ...
          '-fno-omit-frame-pointer', '-fno-rtti','-fno-exceptions','-std=c++11' ]
        ...
        opt.compiler_flags = [ '/Zi', '/W3', '/WX-', '/Ox', '/Ob2', '/Oi', '/Ot',
          '/Oy', '/GL', '/D', 'WIN32', '/D', '_CRT_SECURE_NO_DEPRECATE', '/D',
          '_CRT_NONSTDC_NO_DEPRECATE', '/D', '_HAS_EXCEPTIONS=0', '/D',
          'BUILDING_V8_SHARED=1', '/D', 'BUILDING_UV_SHARED=1',
          '/D', 'BUILDING_NODE_EXTENSION', '/D', '_WINDLL', '/GF', '/Gm-', '/MT',
          '/GS', '/Gy', '/fp:precise', '/Zc:wchar_t', '/Zc:forScope', '/Zc:inline',
          '/GR-', '/Gd', '/TP', '/wd4351', '/wd4355', '/wd4800', '/wd4251',
          '/errorReport:queue', '/MP' /*, /Fd"Release\obj\addon\vc140.pdb"*/ ]
      }
      ...
      this.compile(addonSrcFile, opt, (err) => {
        ...
        if (process.platform === 'darwin') {
          ...
          opt.linker_flags = [ '-shared', '-pthread', '-rdynamic', '-m64',
            `-Wl,-soname=${options.output}.node`, '-Wl,--start-group',
          ...
          opt.linker_flags = [ '/ERRORREPORT:QUEUE', '/INCREMENTAL:NO', 'kernel32.lib',
            'user32.lib', 'gdi32.lib', 'winspool.lib ', 'omdlg32.lib', 'advapi32.lib',
            'shell32.lib', 'ole32.lib', 'oleaut32.lib', 'uuid.lib', 'odbc32.lib',
            'DelayImp.lib', `${process.cwd()}/build/deps/node/${process.versions.node}/node.lib`,
            'Delayimp.lib', '/MANIFEST', `/MANIFESTUAC:"level='asInvoker' uiAccess='false'"`,
            '/manifest:embed', '/MAP', '/MAPINFO:EXPORTS', '/OPT:REF', '/OPT:ICF',
            '/LTCG', '/TLBID:1', '/DYNAMICBASE', '/NXCOMPAT', '/MACHINE:X64', '/ignore:4199', '/DLL'
           ]
        }

        // NOTE: quirk b/c unix executables don't have ending, win have, exlude
        // it manually when building addons
        if (process.platform === 'win32') options.isWinAddon = true
