...
let winBinary
switch (process.arch) {
  case 'x64': winBinary = '\\amd64'; break;
  case 'ia32': winBinary = ''; break;
  case 'arm': winBinary = '\\arm'; break;
}
// default, can be extended only in the class
const compilers = {
  // For easy copy pasting during dev "C:\Program Files (x86)\Microsoft Visual Studio 14.0\VC\bin\amd64\cl.exe"
  win32:   `\"${process.env['ProgramFiles(x86)']}\\Microsoft Visual Studio 14.0\\VC\\bin${winBinary}\\cl.exe\"`,
  freeBSD: 'clang',
  darwin:  'clang',
  linux:   'gcc',
  sunos:   'gcc' // REVIEW
...
class PlatformTools extends EventEmitter {
  ...
  _compile(source, opts, cb) {
    ...
    // actual entry point
    const runner = spawn(this.compiler, args, {stdio: [null ,'inherit', 'inherit'], shell: true});
