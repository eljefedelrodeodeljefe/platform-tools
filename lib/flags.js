...
switch (process.arch) {
  case 'x64':
    // case originates from CI run, were actual process.arch leads to x86 always
    // REVIEW: find workaraound or validate with CI provider
    if (process.env.NO_ARCH) {
      linkerArch = 'X86';
      break;
    }
    linkerArch = 'X64';
    break;
  case 'ia32': linkerArch = 'X86'; break;
